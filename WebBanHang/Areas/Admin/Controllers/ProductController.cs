...
namespace WebBanHang.Areas.Admin.Controllers
{
    ...
    public class ProductController : Controller
    {
        ...
        public ProductController(IProductService service, ICategoryService ceteService, IWebHostEnvironment env, IAuditLogService auditLogService)
        {
            _service = service;
        ...
        [HttpPost]
        public async Task<IActionResult> Index(ProductManagementVM vm, int page = 1)
        {
            var products = await _service.GetAllProductsAsync();
            var categories = await _cateService.GetAllCateAsync();
            ...
            vm.Products = products.Skip((pageIndex - 1) * PageSize).Take(PageSize);
            ...
            ViewBag.TotalPages = totalPages;
            ViewBag.CurrentPage = pageIndex;
            ViewBag.SearchString = vm.SearchInput;
            ViewBag.SortOrder = vm.SortColumn;
        ...
        public async Task<IActionResult> Create(CreateProductVM vm)
        {
            ...
            // Audit Log: Ai tạo sản phẩm
            var createdProduct = await _service.GetProductByCodeAsync(dto.Code);
            await _auditLogService.LogAsync("Product", createdProduct?.ProductId ?? 0, "Create", userId ?? 0, $"Tạo sản phẩm '{dto.Name}' (mã {dto.Code})");
        ...
        public async Task<IActionResult> Edit(UpdateProductVM vm)
        {
            ...
            // Audit Log: Ai sửa sản phẩm
            var editorUserId = HttpContext.Session.GetInt32("UserId") ?? 0;
            await _auditLogService.LogAsync("Product", vm.Id, "Update", editorUserId, $"Cập nhật sản phẩm '{vm.Name}' (mã {vm.Code})");
